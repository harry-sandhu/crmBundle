...
// import CustomReferralNode from "../../components/CustomReferralNode";
...
// frontend/src/components/OrgAndReferralTree.tsx

// frontend/src/components/OrgBinaryThenReferralTree.tsx
// frontend/src/components/OrgBinaryThenReferralTree.tsx
...
type ViewMode = "org-binary" | "referral";

// ----------------- Component -----------------
export default function OrgBinaryThenReferralTree() {
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  const [view, setView] = useState<ViewMode>("org-binary");

  const [orgBinaryRoot, setOrgBinaryRoot] = useState<TreeNode | null>(null);
  const [currentRoot, setCurrentRoot] = useState<TreeNode | null>(null);

  const [nodes, setNodes] = useState<Node<TreeNode>[]>([]);
  const [edges, setEdges] = useState<Edge[]>([]);
  const [selected, setSelected] = useState<TreeNode | null>(null);
  const [history, setHistory] = useState<{ mode: ViewMode; root: TreeNode }[]>([]);

  const nodeTypes = useMemo(() => ({ custom: CustomReferralNode }), []);
  const ORG_ROOT_CODE = "GROLIFE-ROOT-000000";

  // Pure helpers wrapped in useCallback so effects can safely depend on them
  const markAllActive = useCallback(function markAllActive(node: TreeNode): TreeNode {
    return {
      ...node,
      active: true,
      children: (node.children || []).map(markAllActive),
  ...
  const buildBinaryFromSource = useCallback(function buildBinaryFromSource(sourceRoot: TreeNode): TreeNode {
    function collectBFS(root: TreeNode): TreeNode[] {
      const queue: TreeNode[] = [root];
      const bfs: TreeNode[] = [];
      while (queue.length > 0) {
    ...
    const cloned: TreeNode = { ...sourceRoot, children: [...(sourceRoot.children || [])] };

    const waiting = collectBFS(cloned);
    const queue: TreeNode[] = [];

    cloned.children = [];
    while (cloned.children.length < 2 && waiting.length > 0) {
      const next = waiting.shift()!;
      cloned.children.push(next);
      queue.push(next);
    }
  ...
  const convertToFlowBinary = useCallback(function convertToFlowBinary(root: TreeNode): {
    nodes: Node<TreeNode>[];
    edges: Edge[];
  } {
    const nodes: Node<TreeNode>[] = [];
  ...
  const convertToFlowReferral = useCallback(function convertToFlowReferral(root: TreeNode): {
    nodes: Node<TreeNode>[];
    edges: Edge[];
  } {
    const nodes: Node<TreeNode>[] = [];
  ...
  // Fetch Organization tree once, then compute binary and render it
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setErr("");
        const res = await api.get<ApiResponse>(`/api/tree/${ORG_ROOT_CODE}`);
        if (!res.data.success) {
          setErr(res.data.message || "Failed to load organization tree");
          return;
        }
        const orgMarked = markAllActive(res.data.data);
        const binary = buildBinaryFromSource(orgMarked);
        setOrgBinaryRoot(binary);

        setView("org-binary");
        setCurrentRoot(binary);
        const { nodes, edges } = convertToFlowBinary(binary);
        setNodes(nodes);
        setEdges(edges);
      } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
          ...
          setErr(`Failed to fetch organization tree (${status}): ${message}`);
        } else if (error instanceof Error) {
          setErr(`Failed to fetch organization tree: ${error.message}`);
        } else {
          setErr("Failed to fetch organization tree");
        }
      } finally {
        setLoading(false);
      }
    })();
  }, [ORG_ROOT_CODE, markAllActive, buildBinaryFromSource, convertToFlowBinary]);

  // Drill down: on org-binary click -> referral layout; in referral, click -> deeper referral
  const handleNodeClick = useCallback(
    async (nodeData: TreeNode) => {
      if (view === "org-binary") {
        if (orgBinaryRoot) {
          setHistory((h) => [...h, { mode: "org-binary", root: orgBinaryRoot }]);
        }
        await loadReferralByRef(nodeData.refCode);
        return;
      }
      if (view === "referral") {
        if (currentRoot) {
          setHistory((h) => [...h, { mode: "referral", root: currentRoot }]);
        }
        await loadReferralByRef(nodeData.refCode);
      }
    },
    [view, orgBinaryRoot, currentRoot]
  );

  // Fetch a referral tree and render it with multi-child layout
  const loadReferralByRef = useCallback(
    async (refCode: string) => {
      try {
        setLoading(true);
        setErr("");
        const res = await api.get<ApiResponse>(`/api/tree/${refCode}`);
        if (!res.data.success) {
          setErr(res.data.message || "Failed to load referral tree");
          return;
        }
        const tree = res.data.data;
        setView("referral");
        setCurrentRoot(tree);
        const { nodes, edges } = convertToFlowReferral(tree);
        setNodes(nodes);
        setEdges(edges);
      } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
          ...
          const status = axErr.response?.status ?? "network";
          const message = axErr.response?.data?.message ?? axErr.message ?? "Request failed";
          setErr(`Failed to fetch referral tree (${status}): ${message}`);
        } else if (error instanceof Error) {
          setErr(`Failed to fetch referral tree: ${error.message}`);
        } else {
          setErr("Failed to fetch referral tree");
        }
      } finally {
        setLoading(false);
      }
    },
    [convertToFlowReferral]
  );
  ...
  const handleBack = useCallback(() => {
    ...
    setView(prev.mode);
    setCurrentRoot(prev.root);

    if (prev.mode === "org-binary") {
      const { nodes, edges } = convertToFlowBinary(prev.root);
      setNodes(nodes);
      setEdges(edges);
    } else {
      const { nodes, edges } = convertToFlowReferral(prev.root);
      setNodes(nodes);
      setEdges(edges);
    }
    setErr("");
  }, [history, convertToFlowBinary, convertToFlowReferral]);

  if (loading) return <div className="text-center text-green-700 mt-10">Loading tree...</div>;
  if (err) return <div className="text-center text-red-600 mt-10">⚠️ {err}</div>;
  if (!currentRoot) return <div className="text-center text-gray-500 mt-10">No tree data found.</div>;

  return (
    <div className="h-[80vh] w-full bg-green-50 rounded-xl shadow-md border border-green-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-2xl font-bold text-green-700 flex-1 text-center">
          {view === "org-binary" ? "Organization Binary Tree 🌳" : "Referral Tree 🌿"}
        </h2>
        {history.length > 0 && (
          <button
            onClick={handleBack}
      ...
      <ReactFlow
        ...
        nodesDraggable={false}
        nodeTypes={nodeTypes}
        onNodeClick={(_, node) => handleNodeClick(node.data as TreeNode)}
        onNodeDoubleClick={(_, node) => setSelected(node.data as TreeNode)}
      >
        <Background />
        <MiniMap />
        <Controls />
